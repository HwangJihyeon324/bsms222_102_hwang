...
3. The function 'nchar' tells you how many characters long a character vector is. Write a line of code that assigns to the object new_names the state abbreviation when the state name is longer than 8 characters.*추가*
```{r}
if(nchar("murders$state")>8) {
  new_names<-murders$abb
}
```
...
s_n_ <- vector("numeric", 25)
for(n in 1:25){
  s_n_[n] <- compute_s_n(n)
}
n<-1:25
plot(n, s_n_)
```
...
c_n[n] <- n*(n+1)*(2*n+1)/6
plot(n, s_n_-c_n)
## s_n_-c_n = 0 임을 그래프로 확인하였으므로 confirm됨.
```