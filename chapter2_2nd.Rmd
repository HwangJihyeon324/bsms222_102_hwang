...
```
 ...
 R also offers functionns to change from one type to another. For example, you can turn numbers into characters with:
```{r}
x <- 1:5
y <- as.character(x)
y
...
3. Use the names function and the objects defined in the previous exercises to associate the temperature data with is corresponding city.
```{r}
names(temp)<- city
```
...
11. The The class of class(a<-1) is numeric, not integer. R defaults to numeric and to force an integer, you need to add the letter L. Confirm that the class of 1L is integer.
```{r}
a<-1L
class(a)
```
...
## 2.9.2 order
 ...
 The function order is closer to what we want. It take sa vector as input and returns the vvector of indexes that sorts the input vector. This may sound confusing so let's look at a simple example. We can create a vector and sort it:
```{r}
x <- c(31, 4, 15, 92, 65)
sort(x)
``` 
...
```
 ...
 Does this mean California is the most dangerous state? In an upcoming section, we argue that we should be considering rates instead ot totals. Before doing that, we introduce one last order-related function: rank.
...
## 2.9.5 Beware of recycling
 ...
 Another common source of unnoticed errors in R is the use of *recycling.* We saw that vectors are added elementwise. So if the vectors don't match in length, it is natural to assume that we should gent an error. But we don't. Notice what happens:
```{r}
x<- c(1,2,3)
...
# 2.10 Exercises
 ...
 For these exercises we will use the US murder dataset. Make sure you load it prior to starting.
```{r}
library(dslabs)
data("murders")
```
...
4. Now we know how small the smallest state is and we know which row represents it. Which state is it? Define variable 'states' to be the state names for the murders data frame. Report the name of the state with the smallest population.
```{r}
...
```
 Use the rank function to determine the population rank of each state from the smallest population size to biggest. Save these ranks in an object called 'ranks', then create a data frame with the state name and its rank. Call the data frame my_df.
```{r}
ranks <- rank(murders$population)
my_df <- data.frame(name = murders$state, rank = ranks)
```

6. Repeat the previous exercise, but this time order my_df so that the states are ordered from the least populous to most populous. Hint: create an object ind that stores the indexes needed to order the population values. Then use the bracket operator [ to re-order each column in the data frame.
```{r}
ind <- order(murders$population)
my_df <- data.frame(name = murders$state, rank = murders$population[ind])
order(my_df)
```
...
# 2.11 Vector arithmetics
 ...
 California had the most murders, but does this mean it is the most dangerous state? What if it just has many more people than other state? We can quickly confirm that California indeed has the largest population:
```{r}
library(dslabs)
data("murders")
...
```
 ...
 In the line above, we multiplied each element by 2.54. Similarly, if for each entry we want to compute how many inches taller or shorter than 69 inches, the average height for males, we can subtract it from every2entry like this:
```{r}
inches-69
```
...
## 2.11.2 Two vectors
 ...
 If we haave two vectors of the same length, and we sum them in R, they will be added entry by entry as follows:
...
```
 ...
 Once we do this, we notice that California is no longer near the top of the list. In fact, we can use what we have learned to order the staes by murder rate:
```{r}
...
city_temps <- data.frame(name = city, temperature = temp)
```
 Remake the data frame using the code above, but add a line that converts the temperature from Fahrenheit to Celcius. The conversion is C=5/9*(F-32).
```{r}
city_temps <- data.frame(name = city, temperature = 5/9*(temp-32))
```

2. What is the following sum? Hint: thanks to Euler, we know it should be close to π^2/6.
```{r}
num <- seq(1, 100)
class(num)
num_function <- 1/(num^2)
sum(num_function)
```
...
# 2.13 Indexing
 ...
 R provides a powerful and convenient way of indexing vectors. We can, for example, subset a vector based on properties of another vector. In this section, we continue working with our U murders example, which we can load like this:
```{r}
library(dslabs)
data("murders")
```
...
## 2.13.3 which
 ...
 Suppose we want to look up California's urder rate. For this type of operation, it is convenient to convert vectors of logicals into indexes instead of keeping long vectors of logicals. The function which tells us which entries of a logical vector are TRUE. So we can type:
```{r}
...
length(murders$state[murder_rate < murder_mean])
```
...
```{r}
Mabb <- c("MA", "ME", "MI", "MO", "MU")
Mabb[which(c("MA", "ME", "MI", "MO", "MU")%in%murders$abb)]
```
...
```
Keep in mind that many states have populations below 5 million and are bunched up. We may gain further insights from making this plot in the log scale. Transform the variables using the log10 transformation and plot them.
```{r}
population_in_log <- log(murders$population)
total_gun_murders <- murders$total
